...
Safe_Queue::Safe_Queue(size_t size) : Lock_Buffer(size){
	this->queue = new std::queue<double*>();
	return;
}
...
bool Safe_Queue::safe_push(double* const task){
...
bool Safe_Queue::try_safe_push(double* const task){
...
bool Safe_Queue::safe_pop(double **task){
	std::unique_lock<std::mutex> lock(*d_mutex); 
...
bool Safe_Queue::try_safe_pop(double **task){
	std::unique_lock<std::mutex> lock(*d_mutex, std::try_to_lock);
	if(lock.owns_lock()){
		*task = this->queue->front();
		this->queue->pop();
